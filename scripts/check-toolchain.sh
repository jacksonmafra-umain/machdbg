...
if ! capstone_version="$(pkg-config --modversion capstone 2>/dev/null)" || [[ -z "$capstone_version" ]]; then
    printf 'FAIL capstone — brew install capstone\n'
    missing=1
elif [[ "$(printf '%s\n%s\n' "5" "$capstone_version" | sort -V | head -1)" != "5" ]]; then
    printf 'FAIL capstone — found %s, need 5 or newer; brew install capstone\n' "$capstone_version"
    missing=1
else
...
if [[ -z "${QT_ROOT_DIR:-}" ]]; then
    printf 'FAIL Qt6 — set QT_ROOT_DIR to a Qt 6 macOS installation; see docs/COMPILE-macos.md\n'
    missing=1
elif [[ ! -x "$QT_ROOT_DIR/bin/qmake" ]]; then
    printf 'FAIL Qt6 — QT_ROOT_DIR=%s has no bin/qmake\n' "$QT_ROOT_DIR"
    missing=1
else
