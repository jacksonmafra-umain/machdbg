...
| Capstone | BSD-3-Clause | Linked by the native build as of milestone 6: `x64dbg_widgets` finds it through pkg-config (`brew install capstone`, verified by `scripts/check-toolchain.sh`) and the disassembler is built on it. The vendored `src/cross/remote_server` Python prototype lists it separately as a pip dependency (`capstone>=5.0.6`), unrelated to that build |
